...
class ArticleRepository implements ArticleRepositoryInterface
{
    ...
    public function getShortDescriptionByArticle(ArticleApi $article)
    {
        $articleId = $article->getId();

        $shortDescription = null;

        session('postMetas', new Collection())->map(function ($postMeta) use ($articleId, &$shortDescription) {
            if ($postMeta->meta_key == 'education_article_settings_short_description' && $postMeta->post_id == $articleId) {
                $shortDescription = $postMeta->meta_value;
            }
        });
    ...
    public function getImages(ArticleApi $article)
    {
        ...
        // session('postMetas', new Collection())->map(function ($postMeta) use ($thumbId) {
        //     if ($postMeta->meta_key == '_wp_attachment_metadata' && $postMeta->post_id == $thumbId) {
        //         // dd(base64_encode( serialize( "123123" ) ));
        //         // dd($postMeta);
        //         dd(unserialize($postMeta->meta_value));
        //         $array_restored_from_db = unserialize(base64_decode($postMeta->meta_value));
        //         dd($array_restored_from_db);
        //     }
        // });
        ...
        $imagesModel = session('postMetas', new Collection())->map(function ($postMeta) use ($thumbId) {
            if ($postMeta->meta_key == '_wp_attachment_metadata' && $postMeta->post_id == $thumbId) {
                ...
                try {
                    $file = unserialize($postMeta->meta_value)['file'];
                } catch (\Throwable $th) {
                ...
                $arraySizes = unserialize($postMeta->meta_value)['sizes'];

                if ($arraySizes) {
                    $postThumbnailSize = $arraySizes['sp-thumb-600']['file'];

                    array_push($fileParts, $postThumbnailSize);
                    // dd($postMeta);
                    $file = implode('/', $fileParts);
                }
                
                $postMeta->final_img = $file;
